...
(async() => {
    const steps = [
        ...
        () => utils.exec(`Deploying Application`, `ssh -o StrictHostKeyChecking=no -i ./pemfiles/${env.pemfile} ${env.user}@${env.location} ". ~/.bashrc ; source ~/.nvm/nvm.sh ; cd ./dist ; npx pm2 start main.js -i max" > logs/start-pm2.log`),
        () => utils.exec(`Configuring Startup Script`, `ssh -o StrictHostKeyChecking=no -i ./pemfiles/${env.pemfile} ${env.user}@${env.location} ". ~/.bashrc ; source ~/.nvm/nvm.sh ; cd ./dist ; sudo env PATH=$PATH:/home/${env.user}/.nvm/versions/node/v12.4.0/bin /home/${env.user}/dist/node_modules/pm2/bin/pm2 startup systemd -u ${env.user} --hp /home/${env.user}" > logs/startup-pm2.log`),
